...
class OAuthAuthentication implements MiddlewareInterface {
  ...
  public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
    ...
    if ($user === null) {
      ...
      $response = (new ResponseFactory)->createResponse(StatusCodeInterface::STATUS_UNAUTHORIZED)->withHeader(HttpHeader::WWW_AUTHENTICATE, sprintf('Basic realm="%s"', $this->options["realm"]));
      return $this->processError($response, ["message" => "Authentication failed"]);
    } else {
      ...
      if ($request->getAttribute("user") !== null) {
        ...
        return $this->processError($response, ["message" => "Overriding 'user' attribute not allowed"]);
      }
